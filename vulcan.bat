...
:cli-up (
    ...
    IF defined SUBPROJECT (
        docker run -d --rm ^
            ...
            --name vulcan-service-%PROJECT_NAME%-%CONTAINERPORT% ^
            vulcan vulcan %SUBPROJECT%"
    ) else (
        docker run -ti --rm ^
            -v %cd%\src:/vulcan ^
            -p %CONTAINERPORT%:3000 ^
            --name vulcan-service-%PROJECT_NAME% ^
            vulcan bash
    )
...
:cli-up-args (
    set COMMON_ARGS_KEY=%1
    set COMMON_ARGS_VALUE=%2
    if "%COMMON_ARGS_KEY%"=="-d" (set SUBPROJECT=%COMMON_ARGS_VALUE%)
    if "%COMMON_ARGS_KEY%"=="-p" (set CONTAINERPORT=%COMMON_ARGS_VALUE%)
    goto end
)
...
:cli-up-help (
    ...
    echo      -d                The Project directory which need run with vulcan.
    echo      -p                The container port.
    echo.
    goto end
)
...
:cli-down (
    echo ^> Down service
    docker rm -f vulcan-service-%PROJECT_NAME%-%CONTAINERPORT%
    goto end
)
